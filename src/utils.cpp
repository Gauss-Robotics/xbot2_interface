...
{
        std::string urdf_str;
    #ifdef XBOT2IFC_URDFDOM_USES_TINIXML2
        auto tiXmlDoc(urdf::exportURDF(urdf));
        tinyxml2::XMLPrinter printer;
        tiXmlDoc->Accept(&printer);
        urdf_str = printer.CStr();
    #else
        auto tiXmlDoc(urdf::exportURDF(urdf));
        TiXmlPrinter printer;
        tiXmlDoc->Accept(&printer);
        urdf_str = printer.Str();
    #endif

    // HACK: empty texture tags are generated by urdf::exportURDF :(
    // this makes python's urdf parser freak out
    std::string::size_type n = 0;
    std::string s = "<texture />";
    while((n = urdf_str.find(s, n)) != std::string::npos)
    {
        urdf_str.replace( n, s.size(), "");
    }
